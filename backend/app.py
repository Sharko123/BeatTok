...
def generate_audio():
    try:
        ...
        if file and file.filename.endswith('.wav'):
            ...
            main(input_audio_path, drums, bass, 4, output_audio_path)  # Adjust parameters as needed
            print("finished generating")
            # Use absolute path to ensure Flask can find the file
            output_audio_path_abs = os.path.join(os.path.abspath(os.path.dirname(__file__)), output_audio_path)
            if os.path.exists(output_audio_path):
                print("GOT FILE")
                return send_file(output_audio_path_abs, as_attachment=True, mimetype='audio/mpeg')
            else:
